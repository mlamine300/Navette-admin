...
const FilterTableDiv = ({className,stations}:{className?:string,stations?:Station[]}) => {
 const [searchParams,setSearchParams]=useSearchParams();
    ...
    }, [search, type,station, setSearchParams, searchParams]);
    return (
<Accordion
      type="single"
      collapsible
 ...
 <div className={cn("flex flex-col gap-1 md:grid md:grid-cols-2 lg:grid-cols-3 w-full md:min-h-16 md:gap-4",className)}>
   ...
   {stations&&
                ...
                <label className={'w-full flex text-xs italic '} htmlFor={`select-stations`}>"station" </label>
                <Select 
                value={station}
              ...
              >
                ...
                <SelectContent  id={`select-station`} className="bg-background-base ">
                   <p className='text-sm hover:cursor-pointer'  onClick={()=>setstation("")}>
                    station
                  </p>
                  { stations?.map((val) => (
